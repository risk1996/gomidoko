  ...
  return (
    ...
    >
      <svg
        ...
        class="size-4"
      >
        <title>{t.common.actions.close()}</title>
        <path d="M18 6l-12 12" />
        <path d="M6 6l12 12" />
      </svg>
    </ToastPrimitive.CloseButton>
...
const DEFAULT_TOAST_DURATION = day.duration(5, "seconds").asMilliseconds();
    ...
    >
      <Switch>
        ...
        <Match when={props.state === "fulfilled"}>
          {props.data ? options.success?.(props.data) : null}
        </Match>
        <Match when={props.state === "rejected"}>
          {props.error ? options.error?.(props.error) : null}
        </Match>
      </Switch>
    </Toast>
  ));
